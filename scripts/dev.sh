...
try mkdir -p $HOME/dev/{hugo,leka,tmp}
...
try git clone --recursive https://github.com/HPezz/LekaOS
...
tap_path="/opt/homebrew/Homebrew/Library/Taps/osx-cross"

try mkdir -p /opt/homebrew/Homebrew/Library/Taps/osx-cross
...
try gem install --no-document neovim
try python3 -m pip install -U --user mbed-cli mbed-tools pyserial neovim pyocd
